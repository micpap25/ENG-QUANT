...
def ipm_solve_toy(beta: float = 0.1, beta2: float = 1 - 5e-4, omega: float = 1e2,
    ...
    while True:
        ...
        M = np.dot(ATZ1_1Y, A) + np.dot(GTZ_1W, G) + np.diag(gamz4_1 + lamx_1)
        ...
        r = np.dot(ATZ1_1Y, b_Ax) + np.dot(GTZ_1W, h_Gx) + gamz4_1 * u_x \
            - mu * (np.dot(A.T, z1_1) + np.dot(G.T, z2_1 - z3_1) + z4_1 - x_1) \
            - c - ATy - GTw1_w2 - gam
        ...
        while not is_neighbor:
            ...
            compl_temp = np.dot(y_temp, z1_temp) + np.dot(w1_temp, z2_temp) \
                + np.dot(w2_temp, z3_temp) + np.dot(gam_temp, z4_temp) + np.dot(x_temp, lam_temp)
            is_neighbor = True
